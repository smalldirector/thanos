...
func NewReplicaMerger(logger log.Logger, metrics *DedupMetrics, bkt objstore.Bucket, dir string, replicaLabel string,
	blockSyncTimeout time.Duration) *ReplicaMerger {
...
func (rm *ReplicaMerger) Merge(ctx context.Context, resolution int64, replicas Replicas) error {
	...
	for _, group := range groups {
		level.Debug(rm.logger).Log("msg", "starting to dedup blocks", "group", group)
...
func (rm *ReplicaMerger) prepare(ctx context.Context, group *BlockGroup) error {
	...
	if rm.blockSyncTimeout.Seconds() > 0 {
		mCtx, cancel = context.WithTimeout(ctx, rm.blockSyncTimeout)
	} else {
	...
	for _, b := range group.blocks {
		...
		go func(b *metadata.Meta) {
			...
			if err != nil {
				rm.metrics.syncBlockFailures.WithLabelValues(rm.bkt.Name(), b.ULID.String()).Inc()
				errChan <- err
			}
		}(b)
	}
