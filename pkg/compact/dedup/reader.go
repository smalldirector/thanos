...
func (ss *SampleSeries) ToChunkSeries() (*ChunkSeries, error) {
	if len(ss.data) == 0 {
		return &ChunkSeries{lset: ss.lset, chks: nil}, nil
	}
	if ss.res == 0 {
...
func (ss *SampleSeries) toDownsampleChunkSeries() (*ChunkSeries, error) {
	countChks, err := ss.toChunks(CountSample)
	if err != nil {
		return nil, err
	}
	...
	for _, countChk := range countChks {
		...
		sumChk, err := ss.toChunk(SumSample, minTime, maxTime)
		if err != nil {
			return nil, err
		}
		if sumChk != nil {
		...
		minChk, err := ss.toChunk(MinSample, minTime, maxTime)
		if err != nil {
			return nil, err
		}
		if minChk != nil {
		...
		maxChk, err := ss.toChunk(MaxSample, minTime, maxTime)
		if err != nil {
			return nil, err
		}
		if maxChk != nil {
		...
		counterChk, err := ss.toChunk(CounterSample, minTime, maxTime)
		if err != nil {
			return nil, err
		}
		if counterChk != nil {
...
func (ss *SampleSeries) toChunk(st SampleType, minTime, maxTime int64) (*chunks.Meta, error) {
	samples := ss.data[st]
	if len(samples) == 0 {
		return nil, nil
	}
